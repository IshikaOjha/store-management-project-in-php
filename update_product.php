...
if(isset($_POST['submit']))
{
	...
	$amount=$quantity*$cp;
	$supplier=$_POST['supp'];
	$sql="UPDATE products SET code='$code',name='$name',cp='$cp', sp='$sp',quantity='$quantity',amount='$amount',supplier='$supplier' where id=$id";
    $result=mysqli_query($db,$sql);
	if (!$result)
	{
...
<div class="panel panel-heading" align="center" style="background-color:#d5dae2; color:red;"><b>Update Products Details</b></div>
...
<input type="number" class="form-control" name="code" style="width:200px" value="<?php echo$item['code']; ?>"/>

<label>Product name:</label>
<input type="text" class="form-control" name="name" style="width:200px" value="<?php echo$item['name']; ?>"/>

<label>Cost Price:</label>
<input type="text" class="form-control" name="cp" style="width:200px" value="<?php echo$item['cp']; ?>"/>

<label>Selling Price:</label>
<input type="text" class="form-control" name="sp" style="width:200px" value="<?php echo$item['sp'];?>"/>

<label>Quantity:</label>
<input type="number" class="form-control" name="quantity" style="width:200px" value="<?php echo$item['quantity'];?>"/>

<label>Supplier:
<select  class="form-control" style="width:200px" name="supp" >
<?php while($data=mysqli_fetch_assoc($supplier_result)){?>
	<option value="<?php echo $data['name'];  ?>"  <?php if($data['name']==$item['supplier']){echo "selected";}?>><?php echo $data['name']; ?></option>
<?php }?>
</select>
</label><br>
